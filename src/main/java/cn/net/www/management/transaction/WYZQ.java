...
public class WYZQ {
	public static void xunta(ServletContext sc,HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		if(state.equals("begin")){
			page="begin";
			if(pb==null){
			count=QueryList.count(type,YohooUser.WYZQ_REQUEST_START);
			pb=new PageBean();
			pb.setPagenow(1);
			...
			List<Object> list=QueryList.xunta(pb,YohooUser.WYZQ_REQUEST_START);
			request.setAttribute("list",list);
			}
		else if(state.equals("pass")){
				page="pass";
				if(pb==null){
				count=QueryList.count(type,YohooUser.WYZQ_REQUEST_PASS);
				pb=new PageBean();
				pb.setPagenow(1);
				pb.setItem(PageBean.LENGTH_TEN);//1页10个
				pb.setCount(count);
				...
				List<Object> list=QueryList.xunta(pb,YohooUser.WYZQ_REQUEST_PASS);
				request.setAttribute("list",list);
				}
			else if(state.equals("nopass")){
				page="nopass";
				if(pb==null){
				count=QueryList.count(type,YohooUser.WYZQ_REQUEST_NOT_PASS);
				pb=new PageBean();
				pb.setPagenow(1);
				pb.setItem(PageBean.LENGTH_TEN);//1页10个
				pb.setCount(count);
				...
				List<Object> list=QueryList.xunta(pb,YohooUser.WYZQ_REQUEST_NOT_PASS);
				request.setAttribute("list",list);
