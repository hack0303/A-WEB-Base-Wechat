...
public class Activities {
	public static void xunta(ServletContext sc,HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		if(state.equals("begin")){
			page="begin";
			if(pb==null){
			count=QueryList.countActivities(YohooActivities.JOIN);
			pb=new PageBean();;
			pb.setPagenow(1);
			...
			List<Object> list=QueryList.activities(pb, "xunta",YohooActivities.JOIN);
			request.setAttribute("list",list);
			}
		else if(state.equals("pass")){
				page="pass";
				if(pb==null){
					count=QueryList.countActivities(YohooActivities.PASS);
				pb=new PageBean();
				pb.setPagenow(1);
				pb.setItem(PageBean.LENGTH_TEN);//1页10个
				pb.setCount(count);
              ...
              List<Object> list=QueryList.activities(pb, "xunta",YohooActivities.PASS);
				request.setAttribute("list",list);
				}
			else if(state.equals("nopass")){
				page="nopass";
				if(pb==null){
					count=QueryList.countActivities(YohooActivities.NO_PASS);
				pb=new PageBean();
				pb.setPagenow(1);
				pb.setItem(PageBean.LENGTH_TEN);//1页10个
				pb.setCount(count);
				...
				List<Object> list=QueryList.activities(pb, "xunta",YohooActivities.NO_PASS);
                request.setAttribute("list",list);
				}
